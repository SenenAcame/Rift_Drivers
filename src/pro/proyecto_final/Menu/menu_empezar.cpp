...
menu_empezar::menu_empezar(float width, float heigth)
{
    ...
    empezar_opciones[0].setPosition(sf::Vector2f(width/2, heigth/(MAX_NUMBER_OF_ITEMS+1)*1));

    empezar_opciones[1].setFont(font);
    empezar_opciones[1].setFillColor(sf::Color::White);
    empezar_opciones[1].setString("Atrás");
    empezar_opciones[1].setPosition(sf::Vector2f(width/2, heigth/(MAX_NUMBER_OF_ITEMS+1)*2));
}